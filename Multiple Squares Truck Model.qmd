...
parameters <- list(
  ...
  pm_disp = 0.8, #PM dispersal rate, 80% of PM in the air disperses every day
  
  co2 = vehicle_summary$CO2[1], #trucks CO2 emissions rate (g/truck/day)
  
  nox = vehicle_summary$NOx[1] * 10^6 / V / 1.4, #trucks NOx emissions rate (ppb/truck/day/m^3)
...
dY <- c()
 ...
 # rate of change
   ...
   #Y[i + 4N] is NOx
      for (i in 1:N){
        dY[i] <- parameters$b[i] * Y[i + N] -  #trucks
          parameters$a[i] * Y[i] - 
          sum(parameters$C[,i]) * Y[i] +
          sum(parameters$C[i,] * Y[1:N]) 
        
        dY[i + N] <- parameters$w1[i] * Y[i + N] * #warehouses
          (1 - (1 / parameters$w2[i]) * 
             Y[i + N])
