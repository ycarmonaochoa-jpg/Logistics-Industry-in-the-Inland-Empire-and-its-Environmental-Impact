...
for (j in 1:N){
for (i in 1:N){
  Output[i] <- parameters$b[i]*vars[i + N] - parameters$a[i]*vars[i] - parameters$C[[j]][i]*vars[i]# + sum(C[[j]])*vars[i]
   Output[i + N] <- w[i]
}
} 
 # return the rate of change
...
j <- 1:N
 ...
 # rate of change
map2(.x = 1:N, .y = 1:N, .f = ~{
  Output[.x] <- parameters$b[.x]*vars[.x + N] - parameters$a[.x]*vars[.x] - parameters$C[[.y]][.x]*vars[.x]# + sum(C[[j]])*vars[i]
  Output[.x + N] <- 100
}
)
 # return the rate of change
